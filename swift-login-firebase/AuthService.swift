...
class AuthService {
    ...
    ///     - Bool: was resgistered - Determines if the user was registerd and saved in the database correctly.
    ///     -   Error?: An optional error if firebase sent an error
    ...
    public func fetchUser(completion: @escaping (User?, Error?) -> Void) {
        ...
        let db = Firestore.firestore()
        db.collection("users")
            .document(userUID).getDocument { snapshot, error in
                if let error = error {
